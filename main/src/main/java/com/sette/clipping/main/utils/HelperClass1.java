...
public class HelperClass1 {
    ...
    public static Mediums getMediumByName(String mediumName, String mediumNameCyrilic, MediumType mediumType,
        ...
        if (!mediumList.isEmpty()) {
            ...
            //newMedium.setMedium_id(HelperClass.generateRandomId());
            ...
            //newMedium.setMedium_type_id();
            return newMedium;
        }
    }

	/*public static String getMediumNameLinkedClip(Clip clip, Boolean mediumType) {
		ClipSmall c = new ClipSmall();
		c.setMedium(clip.getMedium());
		return getMediumName(c, mediumType);
	}

	public static String getMediumName(ClipSmall clip, Boolean mediumType) {
		try {
			String medium = clip.getMedium().getMediumName();
			if (mediumType == null || mediumType == false) {
				medium = clip.getMedium().getMediumNameCyrillic();
				if (medium == null || medium.isEmpty()) {
					medium = clip.getMedium().getMediumName();
				}
			} else {
				medium = clip.getMedium().getMediumName();
			}
...
//	}
    ...
    /*public static List<ClientSmall> getClientsByFollowType(EntityManagerFactory emf5, String type) {
        ...
        List<ClientSmall> clearClients = new ArrayList<ClientSmall>();
        for (Clients klient : clients) {
            ClientSmall smallClient = new ClientSmall(klient.getIdClient(),
                    klient.getShortName(), klient.getType(),
                    klient.getLongName());
            clearClients.add(smallClient);
        }
        return clearClients;
    }
*/
...
/*
				...
				if(clips != null && !clips.isEmpty()){
					for (Clip c : clips) {
						Predicate newPredicate = mailSender.equal(
								clientRoot.get("clipId"), c.getIdClip());
    ...
    public static Clip checkClipValues(Clip clip) {
        ...
        //Mediums medium = clip.getMedium();
        //String pageNumber = clip.getPageNumber();
		...
		/*if (tenders != null && !tenders.isEmpty()) {
		  ...
		  //Tenderi
			...
			for (Clip clip : tenders) {
				...
				/*if (clip.getIsPonistuvanje() == null
						&& clip.getIsPrilog() == null
						|| clip.getIsPonistuvanje() == false
						&& clip.getIsPrilog() == false) {
					linkENabavki = "http://" + GlobalVariables.SERVER
							+ "/SetteMasterSystem/admin/enabavki.html?emailID="
							+ emailId + "+&link="
							+ clip.getTender().getUrlNaOglas();
					procenetaVrednost = clip.getTender()
							.getProcenetaVrednostBezDDV();
...
/*public static boolean checkIfContaintsBrandTag(Clip clip) {
		...
		List<Tags> tags=clip.getTags();

		if(tags!=null && !tags.isEmpty()){
			for (Tags tag : tags) {
				if(tag.getType()!=null && tag.getType().equals(TAG_BREND)){
					toAdd=true;
					break;
				}
			}
...
/*public static boolean makeScreenShots(Clip clip) {
	...
	/*public static String getMergeClientTagsAndClipTags(List<Tags> clientTags, List<Tags> clipTags){
		...
		for (Tags t1 : clientTags) {
			for (Tags t2 : clipTags) {
				if(t1.getIdTag().equals(t2.getIdTag())){
					tagsWrapper = tagsWrapper + t1.getName() + " ";
					break;
				}
			}
		}
	...
	/*String imageUrl = clip.getMediaPATH();
	System.out.println("image: " + imageUrl);
	if (imageUrl == null){
		imageUrl = GlobalVariables.IMAGES_WEB_LOCATION + "/" + clip.getIdClip() + ".jpg";
...
public static String getClipImageUrlForPDF(Clip clip){
	String image = clip.getMediaPATH();
